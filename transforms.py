...
def get_train_transforms(height=Config.IMG_HEIGHT, width=Config.IMG_WIDTH):
    return A.Compose([
        ...
        A.OneOf([
            # Tập trung vào xoay và nghiêng (phổ biến với biển số)
            A.Affine(scale=(0.9, 1.1), translate_percent=(0.05, 0.05), rotate=(-10, 10), shear=(-5, 5), p=1.0, mode=cv2.BORDER_CONSTANT, value=0),
            A.Perspective(scale=(0.02, 0.08), p=1.0), 
        ], p=0.5),
        A.OneOf([
            A.RandomBrightnessContrast(brightness_limit=0.2, contrast_limit=0.2, p=1.0),
            A.CLAHE(clip_limit=2.0, tile_grid_size=(8, 8), p=1.0), # Giảm clip_limit để tránh quá cháy ảnh
        ], p=0.5),
        A.OneOf([
            A.GaussianBlur(blur_limit=(3, 3), p=1.0), # Với low-res, không nên blur quá mạnh
